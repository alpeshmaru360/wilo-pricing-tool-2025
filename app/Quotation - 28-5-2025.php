...
class Quotation extends Model
{
        ...
        foreach($quotation as $value)
        {
			...
			elseif($value->cart_model_name == 'firefighting')
            {
				...
				$firefightingCartnull = FireFightingCarts::where('quotation_no', '!=' ,$value->quotation_number)
					...
					->first();
				//dd($firefightingCartnull->quotation_number);
                if (is_null($firefightingCart)) {
                //    dd("fire-fighting is null");
				goto elsemode;
                }
                
                $data['simple_article_no'] = $firefightingCart ? $firefightingCart->article_number : '--';
                $data['article_no'] = $firefightingCart ? $firefightingCart->full_article_number : 'null';
                $data['description'] = $firefightingCart ? $firefightingCart->pump_models : '--';
                $data['qty'] = $firefightingCart ? $firefightingCart->qty : '--';
                $data['unit_price'] = $firefightingCart ? $firefightingCart->price : '--';
                $data['total_price'] = $firefightingCart ? $firefightingCart->total_price : '--';
    ...
    public static function fireFightingPumpData($firefightingCart)
    {
        ...
        switch ($firefightingCart->category) {
			...
			default:         // Handle null or unknown categoryreturn 
			[             
				'system_pressure' => '-',
				'impeller_material' => '-',             
				'no_of_pumps' => '1',       
				...
				'no_of_poles' => '-',             
				'efficicency' => '', 
				'motor_brand' => '-', 'pump_article_number' => '-', 
			];
