...
class BoosterSetController extends Controller {
    ...
    public function calculateMechanicalComponent(Request $request) {
        ...
        $panel_stand_price = 0;
        if($panel_height == 400){
            $master_sheet = BoosterMasterSheetMechanicalComponent::where('brand_code', 5)->where('function_code', 110)->whereIn('range', ['1521', '400'])->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
            ...
            $base_frame_length_price = BoosterMasterSheetMechanicalComponent::where('brand_code', $base_frame_length->brand_code)->where('function_code', $base_frame_length->function_code)->where('range', $base_frame_length->range)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
        ...
        elseif ($panel_height > 400 && $panel_height < 1000){
            ...
            if($base_frame_size < 1775) {
                //fails
                $master_sheet = BoosterMasterSheetMechanicalComponent::where('brand_code', 5)->where('function_code', 110)->whereIn('range', ['1775', '8040'])->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
        ...
        if ($range == 1 && (($starter_type == 1) || ($starter_type == 2) || ($starter_type == 6) || ($starter_type == 5))) {
            if ($system_pressure == 'PN16') {
                $power_monitor_flag = BoosterMasterSheetMechanicalComponent::where('brand_code', 20)->where('function_code', 91)->where('range', 140)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
            } elseif ($system_pressure == 'PN25') {
                $power_monitor_flag = BoosterMasterSheetMechanicalComponent::where('brand_code', 20)->where('function_code', 91)->where('range', 280)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
            }
            ...
            //pass
            $power_monitor_flag2 = BoosterMasterSheetMechanicalComponent::where('brand_code', 20)->where('function_code', 93)->where('range', 40)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
            ...
            if ($system_pressure == 'PN16') {
                $power_monitor_flag1 = BoosterMasterSheetMechanicalComponent::where('brand_code', 8)->where('function_code', 92)->where('range', 160)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
            } elseif ($system_pressure == 'PN25') {
                $power_monitor_flag1 = BoosterMasterSheetMechanicalComponent::where('brand_code', 8)->where('function_code', 92)->where('range', 250)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
            }
        ...
        {
            $cable_unit_data = BoosterMasterSheetMechanicalComponent::where('brand_code', 12)->where('function_code', 111)->where('range', $range)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
        }
        else
        {
            $cable_unit_data = BoosterMasterSheetMechanicalComponent::where('brand_code', 14)->where('function_code', 113)->where('range', $range)->select('price', 'description', 'range', 'brand_code', 'function_code')->get();
        }
    ...
    public function calcualtePriceInBOM($noOfPump, $ptpDistanceId, $systemPressure, $mainfold) {
        ...
        if (Schema::hasColumn($tableName, $columnName)) {
            $cpRecords = DB::table($tableName)->select('item_description', 'brand_code', 'function_code', 'range', 'unit_price', $columnName)
                            ->whereNotNull($columnName)->where($columnName, '!=', 0)->get();
            $arrayResult = json_decode(json_encode($cpRecords), true);
            foreach ($arrayResult as $key => $val) {
             
                $price += $this->calculatePriceInItem($val, $columnName, $mainfold, $systemPressure);
            }
        }
        return $price;
    }

    public function calculatePriceInItem($val = [], $columnName, $mainfold, $systemPressure) {
        $price = 0.00;
        if ($systemPressure == 'PN16') {
            ...
            if($boosterCartData == null || $request->qoutation_value != null){
                ...
                if (!empty($bill_of_mechanical)) {
                    foreach ($bill_of_mechanical as $data) {
                        $this->insertBoosterItem($boosterCartId, $data, $data['brand_code'], $data['function_code'], $data['range'], $data['qty'], $data['price']);
                    }
                }
            ...
            foreach ($arrayResult as $key => $val) {
                if ($systemPressure == 'PN16') {
                    if ($mainfold == 'SS316' && $val['function_code'] == 65) { // Sunction
                        ...
                        $this->insertBoosterItem($booster_cart_id, $val, $val['brand_code'], 69, $val['range'], $val[$columnName], $price);
                    } else if ($mainfold == 'SS316' && $val['function_code'] == 67) { //Discharge
                        $price = $this->getMasterSheetPriceData($val['brand_code'], 71, $val['range']); //Qty * price
                        $this->insertBoosterItem($booster_cart_id, $val, $val['brand_code'], 71, $val['range'], $val[$columnName], $price);
                    } else {
                        $price = $this->getMasterSheetPriceData($val['brand_code'], $val['function_code'], $val['range']); //Qty * price
                        $this->insertBoosterItem($booster_cart_id, $val, $val['brand_code'], $val['function_code'], $val['range'], $val[$columnName], $price);
                    }
                } else { // Pn25
                    if ($mainfold == 'SS316' && $val['function_code'] == 66) { // Sunction
                        $price = $this->getMasterSheetPriceData($val['brand_code'], 70, $val['range']); //Qty * price // 2 parameter is equal to brand code
                        $this->insertBoosterItem($booster_cart_id, $val, $val['brand_code'], 70, $val['range'], $val[$columnName], $price);
                    } else if ($mainfold == 'SS316' && $val['function_code'] == 68) { //Discharge
                        if ($this->getMasterSheetPriceData($val['brand_code'], 72, $val['range'])) {
                            $price = $this->getMasterSheetPriceData($val['brand_code'], 72, $val['range']); //Qty * price
                            $this->insertBoosterItem($booster_cart_id, $val, $val['brand_code'], 72, $val['range'], $val[$columnName], $price);
                        } else {
    ...
    public function insertBoosterItem($booster_cart_id, $val = [], $brand_code, $function_code, $range, $columnName, $price) {
    ...
    public function cartItems($cartId) { //$val is itemData
        ...
        // echo "<pre>". print_r($cpBoosterItems,1);
        //die;
        // if (isset($items[0]->boosterCart->adder_ids) && $items[0]->boosterCart->adder_ids != null) {
        //     $ids = explode(",", $items[0]->boosterCart->adder_ids); //Code ids
        //     if ($ids) {
        //         foreach ($ids as $id) {
        //
        //             switch ($id) {
        ...
        //   DB::enableQueryLog();
            if (Schema::hasColumn($tableName, $columnName)) {
                ...
                if ($arrayResult) {
                    ...
                    foreach ($arrayResult as $key => $val) {
                        $price = $this->calculateControlPanelBomInItem($val, $request, $columnName, $boosterCartId, $enclousreAdderItemData,$i,$cpRecords1);
                        $i++;
    ...
    public function calculateControlPanelBomInItem($val = [], $request, $columnName, $boosterCartId, $enclousreAdderItemData,$i,$enclosure_count) {
        $price = 0.00;
        ...
        if ($request->component == 2 && $val['brand_code'] == 1) { // component 2 =  Economic
            if ($this->getMasterSheetPriceCPData(2, $val['function_code'], $val['range'])) {
                ...
                $this->itemSave($val, 2, $val['function_code'], $val['range'], $columnName, $boosterCartId, $price, $unitPrice);
        ...
        } else if ($request->enclosure == 3 && $val['brand_code'] == 5 && $val['function_code'] == 1) { //3 equal GRP
            if ($this->getMasterSheetPriceCPData(31, 63, $val['range'])) {
                ...
                $this->itemSave($val, 31, 63, $val['range'], $columnName, $boosterCartId, $price, $unitPrice);
        ...
        //
            ...
            //                echo $val['range'];
                ...
                $this->itemSave($val, 5, 64, $val['range'], $columnName, $boosterCartId, $price, $unitPrice);
        ...
        //SLIDE 11
            ...
            //                echo $val['range'];
                ...
                $this->itemSave($val, 32, $val['function_code'], $val['range'], $columnName, $boosterCartId, $price, $unitPrice);
            //                echo "stainless  " . $val['brand_code'] . "  price == " . $this->getMasterSheetPriceCPData(5, 64, $val['range']) * $val[$columnName] . "</br>";
            }
            //
            else {
    ...
    public function itemSave($val = [], $brand_code, $function_code, $range, $columnName, $boosterCartId, $totalPrice, $unitPrice, $adderCode = null, $noOfPump = null) {
                    ...
                    case ($id >= 27 && $id <= 36):  //electrical_common_adder_based_on_ampere code
                        ...
                        if ($arrayResult) {
                            foreach ($arrayResult as $key => $val) {
                                ...
                                $this->itemSave($val, $val['brand_code'], $val['function_code'], $val['range'], $val[$column], $boosterCartId, $price, $unitPrice, $column);
                            }
                        }
                        break;
                        ...
                        if ($arrayResult) {
                            foreach ($arrayResult as $key => $val) {
                                if ($component == 2 && $val['brand_code'] == 1)
                                {
                                    ...
                                    $unitPrice = $this->getMasterSheetElectricalPriceData($val['brand_code'], $val['function_code'], $val['range']);
                                    $encloureArea += $this->getMasterSheetHeightMultiplyByWidth($val['brand_code'], $val['function_code'], $val['range']) * $val[$column];
                                    $this->itemSave($val, $val['brand_code'], $val['function_code'], $val['range'], $val[$column], $boosterCartId, $price, $unitPrice, $column, $noOfPump);
                            }
                        }
                        break;
    ...
    public function searchCalculateMechanicalComponent(Request $request) {
        ...
        // $pump_unit1 = json_encode($pump_unit);
        // $pump_unit_price = json_decode($pump_unit1)->original->data->price;
        ...
        if (str_starts_with($model, 'C')) {
            ...
            //cable_unit_price = 3.31
            
            $cablePrice = $cable_unit_price * ($Cablelength/1000); //CABLE PRICE
            // dd($cablePrice);27.2934
            $overhead = DB::table('setup_fields')->where('name', 'booster_overhead')->pluck('value')[0]; //get from admin
            // dd($overhead);1.13
            $intercompany_margin = $interCompanyMargin; 
            // dd($intercompany_margin);1.0
            $standard_component_price = $this->calcualtePriceInBOM($no_of_pumps, $ptd_distance_id, $system_pressure, $manifold);
            //$standard_component_price = 1288.095
            //$base_frame_size_price = 170.57
            //$power_monitor_flag_price = 65.66
            //$mechanical_code_price = 705.17
            //$panel_stand_price = 85.0
            $mechanical_system_price = $standard_component_price + $base_frame_size_price + $power_monitor_flag_price + $mechanical_code_price + $panel_stand_price;  
            //$mechanical_system_price = 2314.495
            $booster_price = ((($pump_unit_price * $no_of_pumps) + $cp_price + $code_price + ($mechanical_system_price + $cablePrice)) * $overhead ) / $intercompany_margin;
            //$booster_price = 9596.652543147
            //$pump_unit_price = 969.07
            //$no_of_pumps = 3
            //$cp_price = 3227.265696199636
            //$code_price = 16.348773841962
            //$mechanical_system_price = 2314.495
            //$cablePrice = 27.2934
    ...
    public function searchByArticleCalculatePriceInItem($val = [], $component, $enclousre, $starterType, $enclousreItem, $columnName){
        $price = 0.00;
                    ...
                    case ($code == 65 ):  //mechanical_adder_common_pressure_vessel code
                        //if ($request->code65 && !empty($request->code65)) {
                            $cpRecords = DB::table('mechanical_adder_common_pressure_vessel')
                                    ...
                                    ->where('id', $request->code65)
                                    ->first();
                                    // dd($cpRecords,$request->code65,$code65,"test");
                            $arrayResult = json_decode(json_encode($cpRecords), true);
                            //date : 8-4-2022 change:- remove * $code61 from below price formula
                            $price += $this->getMasterSheetPriceData($arrayResult['brand_code'], $arrayResult['function_code'], $arrayResult['range']);
                            $this->insertAdderBoosterItem($booster_cart_id, $arrayResult['brand_code'], $arrayResult['function_code'], $arrayResult['range'], $price, $code);
