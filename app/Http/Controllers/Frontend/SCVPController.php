...
class SCVPController extends Controller {
    ...
    public function index() {
        ...
        return view('frontend.scvp_pump.index', compact('power', 'voltage', 'efficiency', 'poles', 'brand', 'frequency'))->with(
    ...
    public function cartItems($cartId,$returnDataOnly = false) { 
        ...
        if($returnDataOnly) {
            $scpCart = ScpCart::where('id',$cartId)->first();
            $article_number = DB::table('scp_pump_types')->where('id',$scpCart->pump_id)->pluck('bare_shaft_article_number')->first();
            $motor_price =  DB::table('scp_master_motor_prices')
                            ->where('brand',$scpCart->brand)
                            ->where('power',$scpCart->power)
                            ->where('no_of_pole',$scpCart->no_of_pole)
                            ->where('frequency',$scpCart->frequency)
                            ->where('voltage',$scpCart->voltage)
                            ->get();
                            if($scpCart->application == 2)             {     
