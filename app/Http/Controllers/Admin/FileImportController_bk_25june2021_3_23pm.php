...
//        dd('dd');
        $ranges = Range::all();
        $folderName = ControlPanel::select('folder_name', 'file_name_under_folder')
                        ->groupBy('folder_name')->get();


        dd($folderName);
        return view('admin.file-import.import', compact('ranges', 'folderName'));
    }
...
//        dd('dd');
    ...
    public function upload(Request $request) {
        ...
        $folderName = Range::find($request->range)->value;
        $fileName = Range::find($request->range)->value;
        ...
        if (!empty($file)) {
            ...
            $fileName = preg_replace('/\..+$/', '', $file_excel); //Remove Extension
            $tableName = str_replace("-", " ", preg_replace('!\s+!', ' ', $fileName));
            $tableName = strtolower($rangeName . "_" . preg_replace('!\s+!', '_', $tableName));
