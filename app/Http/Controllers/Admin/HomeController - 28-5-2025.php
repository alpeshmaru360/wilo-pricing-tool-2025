...
class HomeController
{
    ...
    public function newExportQuotation(Request $request)
    {
        ...
        $start_date = $selected_year . '-01-01'; // Start date of the selected year
        $end_date = $selected_year . '-12-31'; // End date of the selected year
       ...
       if ($content == 'movefile') {
        ...
        //     return [
        //         'success' => 1,
        //         'currentPage' => $quotation->currentPage(),
        //         'nextPage' => (int)last(explode('=', $quotation->nextPageUrl())),
        //         'lastPage' => $quotation->lastPage(),
        //         'content' => 'old',
        //         'filename' => '',
        //         'msg' => 'Processing...',
        //         'progress' => 'show',
        //         'percentage' => round(100 - ((($quotation->lastPage() - $quotation->currentPage()) / $quotation->lastPage()) * 100), 2)
        //     ];
        // }   
		//dd($quotation);	
        ...
        if (count($quotation) == 0) {
			...
			$filePath = 'QuotationLog/'.$filename; // Relative path for storage// Store the file correctly in public disk
			Excel::store(new QuotationExportDownload($data), $filePath, 'public');
            return [
                'success' => 2,
                //'url' => url('storage/app/public/QuotationLog').'/'.$filename
            ...
            // return Excel::download(new QuotationExportDownload($data), 'quotations.xlsx');
            // // If File Not create
            // if ($filename == '') {
            //     $destinationPath = storage_path('app/public/QuotationLog').'/';
            //     if (!file_exists($destinationPath)) {
            //         mkdir($destinationPath, 0777, true);
            //     }
            //     $filename = 'Quotation-'.date('Y-m-d-H-i-s').'.xlsx';
            //     (new QuotationExportDownload)->queue('public/QuotationLog/'.$filename);
            // }
            ...
            // $sheet->fromArray($data, null, 'A' . $newRowIndex);
		...
		if (count($quotation) > 0) {
			...
			$filename = 'quotations.xlsx';
			$filePath = 'QuotationLog/'.$filename;
			//Excel::store(new QuotationExportDownload($data), $filePath, 'public');
			return Excel::download(new QuotationExportDownload($data), 'quotations.xlsx');
            return [
                'success' => 2,
