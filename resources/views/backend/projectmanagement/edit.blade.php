...
@section('content')
    ...
    <input type="hidden" value=<?php if (isset($_SERVER['HTTP_REFERER']))
    ...
    <div class="card">
            ...
            <div class="row">
                ...
                <div class="col-sm-7 text-right">
                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModal1"
                        onclick=show_products()><i class="fas fa-plus-circle"></i> Add/Edit Products</button>
                    {{--<button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModal"
                        onclick=showuser()><i class="fas fa-plus-circle"></i> Add Sales Representatives</button>--}}
        ...
        ->autofocus() }}
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <select name="project_segment" id="project_segment" class="form-control">
                                    <option value="0" id="ps">Select Project Segment</option>
                                    @if(!empty($data[0]->project_segment))
                                        @foreach($project_segment as $d)
                                            {{-- <option>Project Segment</option> --}}
                                            <option <?php        echo ($data[0]->project_segment == $d->id ? 'selected' : ''); ?>
                            ...
                            <div class="form-group">
                                <select multiple name="apptype[]" id="apptype" class="form-control">
                                    @foreach($get_application as $d)
                                        <option {{ in_array($d->id, explode(',', $data[0]->application_type_ids)) ? 'selected' : '' }} value="{{$d->id}}">{{$d->name}}</option>
                                    @endforeach
                        ...
                        <div class="col-md-6">
                            {{ html()->label(__('Sub Application Type<span class="text-danger"></span>'))->class('form-control-label subapp')->for('subapptype') }}
                            <div class="form-group">
                                <select multiple name="subapptype[]" id="subapptype" class="form-control">
                                    @foreach($name as $key => $value)
                                        <optgroup label="{{trim($key)}}">
                                            @foreach($value as $v)
                                                <option {{ in_array($v->id, explode(',', $data[0]->sub_application_type_ids)) ? 'selected' : '' }} value="{{$v->id}}">{{$v->name}}</option>
                                            @endforeach
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                <select multiple name="typeproject[]" id="typeproject" class="form-control">
                                    @foreach($project_type as $d)
                                        ...
                                        <option {{ in_array($d->id, explode(',', $data[0]->type_of_project_ids ?? '')) ? 'selected' : '' }} value="{{ $d->id }}">{{ $d->name }}</option>
                            ...
                            <div class="form-group">
                                <select name="country" id="country" class="form-control">
                                    @foreach($country as $d)
                                        ...
                                        <option <?php    echo ($data[0]->country_id == $d->id ? 'selected' : ''); ?>
                                            value="{{$d->id}}">{{$d->name}}</option>
        ...
        ->autofocus() }}
                                    ...
                                    <select name="client" id="client" class="form-control">
                                        ...
                                        @if(!empty($data[0]->client_id))
                                            @foreach($stakeholder_client as $d)
                                                ...
                                                @if($d->type == 1)
                                                    <option <?php            echo ($data[0]->client_id == $d->id ? 'selected' : ''); ?>
                                                        value="{{$d->id}}">{{$d->name}}</option>
                                                @endif
                                                {{-- @if($d->type == 1) --}}
                                                {{-- <option value="{{$d->id}}">{{$d->name}}</option> --}}
                        ...
                        <div class="col-md-6">
                            <div class="img_left">
                                <span id="contractorimg">
                                    {{-- <img src="{{url('logo/contractor_logo/'.$data[0]->stakeholdercontractor->logo)}}"
                                    ...
                                    <select name="contractor" id="contractor" class="form-control">
                                        ...
                                        @if(!empty($data[0]->contractor_id))
                                            @foreach($stakeholder_contractor as $d)
                                                ...
                                                @if($d->type == 3)
                                                    <option <?php            echo ($data[0]->contractor_id == $d->id ? 'selected' : ''); ?>
                                                        value="{{$d->id}}">{{$d->name}}</option>
                                                @endif
                                                {{-- @if($d->type == 3) --}}
                                                {{-- <option value="{{$d->id}}">{{$d->name}}</option> --}}
                        ...
                        <div class="col-md-6">
                            <div class="img_left">
                                <span id="consultantimg">
                                    {{--<img src="{{url('logo/consultant_logo/'.$data[0]->stakeholderconsultant->logo)}}"
                                        height="64px" width="64px">--}}
                                </span>
                            </div>
                            <div class="field_content">
                                    ...
                                    <select name="consultant" id="consultant" class="form-control">
                                        ...
                                        @if(!empty($data[0]->consultant_id))
                                            @foreach($stakeholder_consultant as $d)
                                                @if($d->type == 2)
                                                    <option <?php            echo ($data[0]->consultant_id == $d->id ? 'selected' : ''); ?>
                                                        value="{{$d->id}}">{{$d->name}}</option>
                                                @endif
                                                {{-- @if($d->type == 2) --}}
                                                {{-- <option value="{{$d->id}}">{{$d->name}}</option> --}}
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <select name="factory_manufacturer[]" id="factory_manufacturer" class="form-control"
                                    multiple>
                                    @if(!empty($data[0]->factory_manufacturer_ids))
                                        @foreach($factory_manufacturer as $fm)
                                            <option {{ in_array($fm->id, explode(',', $data[0]->factory_manufacturer_ids)) ? 'selected' : '' }} value="{{$fm->id}}">{{$fm->name}}</option>
                                        @endforeach
                                    @else
                                        @foreach($factory_manufacturer as $fm)
                                ...
                                <div class="form-group">
                                    <select id="select-manufacturer_year" name="manufacturer_year" class="form-control">
                                        @foreach ($years as $year => $label)
                                            <option value="{{ $year }}" {{ old('manufacturer_year', isset($data[0]->manufacturer_year) ? $data[0]->manufacturer_year : '') == $year ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-md-6">
                            <div id="result" style="color:green">
                                @if(!empty($data[0]->project_image))
                                    <button type="button"
                                        onclick="deleteImage('wilo_project', <?php    echo $data[0]->id?>)">X</button>
                                    <div class="img_left">
                                        <span id="projectimage"><img
                                                src="{{url('logo/project_details/main_image/' . $data[0]->project_image)}}"
                                                height="64px" width="64px"></span>
                                    </div>
                                @endif
                            ...
                            <div class="field_content">
                                ...
                                <div class="form-group cs_file_btn"><span class="up_btn btn-info"><i
                                            class="fas fa-cloud-upload-alt"></i> Upload Project Image</span>
                                    {{ html()->file('projectimage')}}

                                </div>
                                <span style="font-size:10px" class="text-danger"> (Min. Width: 800px, Min. Height: 400px
                                    required). </span>
                                <br>
                                <span style="font-size:10px" class="text-danger"> (Width: 1330px, Height: 550px suggested).
                                </span>
                                <br>
    ...
    <div class="modal my_modal fade" id="myModal1" role="dialog" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <span class="hvr_br"><button type="submit" class="btn btn-info" data-dismiss="modal"
                            onclick=checkmate()>Add</button></span>
                    </form>
                    <span class="hvr_br"><button type="button" class="btn btn-danger"
                            data-dismiss="modal">Close</button></span>
                </div>
    ...
    <div id="namer">
        ...
        <div style="background:#fff;" class="table-responsive" id="dt" style>
            <table class="table table-hover" id="productdata" style="display: <?php if (empty($res))
        echo 'none' ?>">
    ...
    <script type="text/javascript">
                        ...
                        if (products.includes(value.id)) {
                            ...
                            <li class="form-group row align-items-center">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" step=any id ="quantity_for_`+value.id+`" value=`+quantity[0].quantity+` name="quantity_of_`+value.id+`"placeholder="Quantity">
                                    <input type="number" step=any id ="max_head_for_`+value.id+`" value=`+quantity[0].max_head+` name="max_head_of_`+value.id+`"placeholder="Max. Head">
                                    <input type="number" step=any id ="max_flow_for_`+value.id+`" value=`+quantity[0].max_flow+` name="max_flow_of_`+value.id+`"placeholder="Max. Flow"> 
                                </div>
                            </li>
                         ...
                         `);*/
                                ...
                                $("#show_products").append(`
                                            ...
                                            <li class="form-group row align-items-center">
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="col-md-6 show_product_input d-flex justify-content-end">
                                                    <input type="number" step=any id ="quantity_for_`+ value.id + `" value=` + quantity[0].quantity + ` name="quantity_of_` + value.id + `"placeholder="Quantity">
                                                </div>
                                            </li>
                            ...
                            /*$("#show_products").append(`
                                <li id="dtx" class="form-group row align-items-center">
                                    ...
                                    <div class="col-md-6 show_product_input">
                                        <input type="number" step=any id ="quantity_for_` + value.id + `" name="quantity_of_` + value.id + `" style="display:none;" placeholder="Quantity">
                                        <input type="number" step=any id ="max_head_for_` + value.id + `" name="max_head_of_` + value.id + `" style="display:none;" placeholder="Max. Head">
                                        <input type="number" step=any id ="max_flow_for_` + value.id + `" name="max_flow_of_` + value.id + `" style="display:none;" placeholder="Max. Flow"> 
                                    </div>
                                </li>
                            `);*/
        ...
        function quantity(val) {
            ...
            console.log(x);
            if (x == true) {
                document.getElementById("quantity_for_" + val).style.display = "inline-block";
                // document.getElementById("max_head_for_" + val).style.display = "inline-block";
                // document.getElementById("max_flow_for_" + val).style.display = "inline-block";
            } else {
        ...
        function checkmate() {
            ...
            var selected = [];
            $('#show_products input:checked').each(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.length != 0) {
                        ...
                        $('#productdata').show();
                        $.each(data, function (key, value) {
                            $("#dtr").append("<tr><td>" + value.name[0].name + "</td><td>" + (value[0].quantity == null ? '-' : value[0].quantity) + "</td></tr>");
                            // $("#dtr").append("<tr><td>"+value.name[0].name+"</td><td>"+(value[0].quantity == null ? '-' : value[0].quantity)+"</td><td>"+(value[0].max_head == null ? '-' : value[0].max_head)+"</td><td>"+(value[0].max_flow == null ? '-' : value[0].max_flow)+"</td></tr>");
                        });
                    }
                    else {
                        $('#productdata').hide();
        ...
        $(function () {
            ...
            //         //select.empty();
            //         $("#subapptype").select2("val", "");
            //         //select.append('<option value="0" id="sub">Select Sub Application</option>');    
            //         $.each(data, function(key, value) {
            //             alert("here");
            //             // select.append('<option value=' + value.id + '>' + value.name + '</option>');
            //             var newOption = new Option(value.name, data.id, false, false);
            //             $('#subapptype').append(newOption).trigger('change');
            //         });
        ...
        $(document).ready(function () {
            ...
            $('#apptype').on('select2:close', function (evt) {
                ...
                if ($(this).val() == '') {
                    ...
                    var strIds = "{{$data[0]->sub_application_type_ids}}";
                    $.get(url, function (data) {
                        if (data != 0) {
                            //var select = $('#subapptype');
                            $('#subapptype').empty().trigger('change');
                            ...
                            $.each(data, function (key, value) {
                                ...
                                $.each(value, function (i, d) {
                                    html += '<option ' + (strIds.includes(d.id) ? 'selected' : '') + ' value=' + d.id + '>' + d.name + '</option>'
                                })
                                // html += "<optgroup label='"+key+"'>";
                                // $.each(value, function(i, d){
                                ...
                                // $('#subapptype').append('<option '+ ( strIds.includes(value.id) ? 'selected':'') +' value=' + value.id + '>' + value.name + '</option>').trigger('change');
                            });
                            //$('#subapptype').trigger('change');
                        } else {
                            $('#subapptype').empty().trigger('change');
