@php
    $tool_tip = DB::table('tool_tip')->where('part_id',4)->get();
...
<style>
    ...
    #pump_model_required_size::placeholder{
            ...
            <div class="formWidget">
                ...
                <form action="" id="atmos_form">
                    ...
                    <div class="formFields">
                        <div class="helpBtnWrap" style="position: relative;">
                            <a href="" class="helpBtn">?</a>
                            <div class="popper-content hide">{{$tool_tip[3]->power_supply ?? ""}}</div>
                        </div>
                        <span class="formArrowIcon"><img src="{{asset('fassets/images/arrowDownIcon.png')}}" /></span>
                    ...
                    <div class="formFields">
                        <div class="helpBtnWrap" style="position: relative;">
                            <a href="" class="helpBtn">?</a>
                            <div class="popper-content hide">{{$tool_tip[4]->frequency ?? ""}}</div>
                        </div>
                        <span class="formArrowIcon"><img src="{{asset('fassets/images/arrowDownIcon.png')}}" /></span>
                    ...
                    <div class="formFields">
                        <div class="helpBtnWrap" style="position: relative;">
                            <a href="" class="helpBtn">?</a>
                            <div class="popper-content hide">{{$tool_tip[5]->no_of_poles ?? ""}}</div>
                        </div>
                        <span class="formArrowIcon"><img src="{{asset('fassets/images/arrowDownIcon.png')}}" /></span>
                    ...
                    <div class="formFields">
                        <div class="helpBtnWrap" style="position: relative;">
                            <a href="" class="helpBtn">?</a>
                            <div class="popper-content hide">{{$tool_tip[6]->efficiency ?? ""}}</div>
                        </div>
                        <span class="formArrowIcon"><img src="{{asset('fassets/images/arrowDownIcon.png')}}" /></span>
                    ...
                    <div class="formFields">
                        <div class="helpBtnWrap" style="position: relative;">
                            <a href="" class="helpBtn">?</a>
                            <div class="popper-content hide">{{$tool_tip[7]->motor_brand ?? ""}}</div>
                        </div>
                        <span class="formArrowIcon"><img src="{{asset('fassets/images/arrowDownIcon.png')}}" /></span>
                    ...
                    <div class="formFields">
                        <div class="helpBtnWrap" style="position: relative;">
                            <a href="" class="helpBtn">?</a>
                            <div class="popper-content hide">{{$tool_tip[8]->application ?? ""}}</div>
                        </div>
                        <span class="formArrowIcon"><img src="{{asset('fassets/images/arrowDownIcon.png')}}" /></span>
...
<div id="bareshaftModal" class="modal">
    <!-- Modal content -->
    <div class="modal-content">
      <!-- <span class="close">&times;</span> -->
        <div class="modal-body" id="master-price-record1">
            <!--Table-->
        </div>
        <div class="modalBtns">
            <button id="addtocartBareshaft">Add to Cart</button>
            <span class="close" onclick="refresh()">Cancel</span>
        </div>
    </div>
</div>
    ...
    <div class="modal-content">
      <!-- <span class="close">&times;</span> -->
        ...
        <div class="modalBtns">
            ...
            <span class="close" onclick="refresh()">Cancel</span>
            <button type="button" class="close-cart-modal" id="close_model">Close</button>
        </div>
    </div>
</div>
...
<div id="manual_acessories_price_modal" class="modal">
    <div class="modal-content" id="">
      <!-- <span class="close">&times;</span> -->
        ...
        <div class="modalBtns">
            <!--<button id="insert_price_accessories"> Price</button>-->
...
<script>
    ...
    $("#close_model").on("click", function () {
        $("#manual_price").attr("data-dismiss", "modal");
    });
    ...
    //
    $("#impeller_material").on('change', function (e) {
        $("#loader").show();
        $("#calculate").attr('disabled', 'disabled')
        ajax_data = {
            'impeller_id': $(this).val(),
            'pump_model': $("#pump_model").val()
        };
        var url = "{{url('atmos_giga_price')}}";
        $.ajax({
            type: "post",
            url: url,
            ...
            success: function (res) {
                if (res != "price not found") {
                    ...
                    $("#pump_model_flow_head").css('display','flex');
                    $("#pump_model_impeller_required").css('display','flex');
                    $("#price_hidden").val(res.get_price);
                    $("#is_bare_shaft_price_manual").val(0);
                    $("#pump_model_impeller_size").val(res.standard_impeller_size);
                    $("#impeller_minimum_size").val(res.impeller_minimum_size);
                    $("#impeller_maximum_size").val(res.impeller_maximum_size);
                } else {
                    $('#manual_price').show();
                }
            }
    ...
    $("#insert_price_accessories").click(function () {
        ...
        alert($("#manual_acessories_price").val());
    ...
    $("#calculate").on('click', function () {
        ...
        if(bareshaft_pump_full_pump != "bareshaft_pump"){
            ...
            var manual_price = $("#manual_acessories_price").val();
            ...
            var bare_shaft_article_number = $("#bareshaft_pump_article_number").val();
            var pump_model = $("#pump_model").val();
            var impeller_material = $("#impeller_material").val();
            var bare_shaft_method = $("#is_bare_shaft_article_number_method").val();
            var pump_model_impeller_size = $("#pump_model_impeller_size").val();
            var pump_model_required_size = $("#pump_model_required_size").val();
            $.ajax({
                type: "get",
                url: "{{url('atmos/ajaxCalculateBareshaft')}}",
                data: {
                    'bare_shaft_article_number' : bare_shaft_article_number,
                    'pump_model' : pump_model,
                    'impeller_material' : impeller_material,
                    'bare_shaft_method':bare_shaft_method,
                    'pump_model_impeller_size' : pump_model_impeller_size,
                    'pump_model_required_size' : pump_model_required_size
                },
                success: function (response) {
                    if (response) {
                        $(document).ajaxComplete(function () {
                        ...
                        $("#price").html(response.data.bare_shaft_price);
                        $("#total-price").val(response.data.bare_shaft_price);
                        $("#master-price-record1").html('');
                        $("#master-price-record1").html(response.data.cp_records_html);
                        $("#bareshaftModal").show();
                    }
                },
                error: function (data) {
    ...
    $(document).on("click","#addtocartBareshaft",function(){
        var bare_shaft_article_number_method = $("#is_bare_shaft_article_number_method").val();
        var adderIds = [];
                ...
                if(origin_country == "ksa" && countryOrigin && valueCountry == undefined){
                     ...
                     $('#Select_country_origin').on('click', function() {
                        ...
                        if(!countryOrigin){
                            ...
                            $.ajax({
                                type: "post",
                                url: "{{url('atmos/addToCartBareshaft')}}",
                                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                                data: $('#atmos_form').serialize() + "&adder_ids=" + adderIds + "&country=" + selectedCountry 
                                + "&bare_shaft_article_number_method=" + bare_shaft_article_number_method,
                                success: function (response) {
                                    if (response.url) {
                                        location.reload();
                                    }
    ...
    $("#optional-button").on('click', function () {
        ...
        if (isEmpty) {
            ...
            if (last_master_id == '' || last_master_id != master_id) {
                ...
                $.ajax({
                    type: "post",
                    url: "{{url('atmos/ajax-optional-modal')}}",
    ...
    $(document).on("click", '#optional-button-add', function (event) {
        ...
        if (adderIds.length >= 1) {
            ...
            $.ajax({
                ...
                data: $('#atmos_form').serialize() + "&adder_ids=" + adderIds + "&master_id=" + master_id,
                // contentType: false,
                success: function (response) {
    ...
    //Code for enter full article number 
    $("#full_article_number").bind('keypress',function(e){
        if(e.which == 13){
            if($(this).val().length > 1)
            {
                ...
                $.ajax({
                type: "get",
                url: "{{route('atmos.searchbyarticle')}}",
    ...
    $("#bareshaft_pump_article_number").bind('keypress',function(e){
        if(e.which == 13){
            if($(this).val().length > 1)
            {
                e.preventDefault();
                $("#is_bare_shaft_article_number_method").val("search");
                var bareshaft_pump_article_number = $("#bareshaft_pump_article_number").val();
                $.ajax({
                type: "get",
                url: "{{route('atmos.searchbyarticle')}}",
                data: {
                        bareshaft_pump_article_number: bareshaft_pump_article_number,
                        searchByBarePumpArticleNumber : 1
                      },
                success: function (response){
                    if(response.data.cp_records_html) {
                        $(document).ajaxComplete(function () {
                        ...
                        const pumpTypes = @json($pump_types); // Convert Blade variable to JavaScript variable
                        // Create a mapping of pump names to their ids
                        const pumpMap = {};
                        pumpTypes.forEach(pump => {
                            pumpMap[pump.name] = pump.id;
                        });

                        const pumpName = response.data.pump_name;
                        const impellerMaterial = response.data.impeller_material;
                        $("#pump_model option").each(function(){
                            if($(this).text().trim() === pumpName.trim()){
                        ...
                        $("#impeller_material option").each(function(){
                            if($(this).text().trim() === impellerMaterial.trim()){
                                $(this).prop('selected', true);
                                return false; // Break the loop
                            }
                        });
                        ...
                        $("#pump_model_flow").val(response.data.flow);
                        $("#pump_model_head").val(response.data.head);
                        $("#pump_model_impeller_size").val(response.data.impeller_standard_size);
                        $("#pump_model_required_size").val(response.data.required_impeller_size);
