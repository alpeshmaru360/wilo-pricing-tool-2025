...
<script>
                ...
                if (main_panel_selection == 'electrical' || main_panel_selection == 'electrical-diesel') {
                    ...
                    if (changed == 'electrical_speed' || changed == 'electrical_motor_power') {//20250108 add motor power field in electrical flow
                        ...
                        if($('#electrical_control_panel_type').val() == '')
                        {
                            ...
                            console.log(electrical_control_panel_type);
                            //for (var i = 0; i <= selected_options; i++) {
            ...
            if (changed == '') {
                ...
                if (dieselSelectedData != '') {
                    ...
                    if (dieselFilterData.length > 0) {
                        ...
                        console.log("Diesel selection data:", diesel_selction_data);
                        console.log("Type of data:", typeof diesel_selction_data);

                        // Change New Select options
                        $('#' + new_changed).html(diesel_selction_option.join(''));
                    } else {
                        alert('Data not found. test2');
                    }
                }
    ...
    <script>
        ...
        $(document).on('click', '#optional-button', function() {
            ...
            //
            const savedAdderIds = [...adderIds];
            console.log('Saved adderIds before modal open:', savedAdderIds);
            ...
            if (optional_check) {
                ...
                if (pump_type == 'electrical' || pump_type == 'electrical-diesel') {
                    option_tr = '';
                    electrical_control_panel_type_val = $('#electrical_control_panel_type').val();

                    //20250109 start - All adder ids not coming
                    $.each(adder_ids, function(key, value) {
                        // checked_adder = '';
                        checked_adder = savedAdderIds.includes('' + value.id) ? 'checked' : '';
                        if (value.version == 'FireFighting/Electrical') {
                            if (jQuery.inArray('' + value.id, adderIds) !== -1) {
                                checked_adder = 'checked';
                            }
                            // if (electrical_control_panel_type_val == 'Soft Starter' || electrical_control_panel_type_val == 'Soft Starter + ATS') {
                            //     if (value.id < 9) {
                            ...
                            if (electrical_option_show) {
                                ...
                                option_tr += `<tr>
                                    ...
                                    <td><input type="checkbox" name="adder_id" id="${pump_type}-optional-${value.id}" class="adder-checkbox ${class_name}" data-id="${value.id}" value="${value.id}" ${checked_adder}></td>
                                </tr>`;
                            }
                        }
                    });
                    //20250109 end - All adder ids not coming
        ...
        $(document).on('change', '.group1', function() {
            if ($(this).is(':checked')) {
                if (pump_type === "electrical-diesel") {
                    // Only deselect .group1 checkboxes within the same table
                    $(this).closest('table').find('.group1').not(this).prop('checked', false);
                } else {
                    // Original behavior for other pump types
                    $('.group1').not(this).prop('checked', false);
                }
            }
        });
        ...
        $(document).on('click', '#jockey-optional-button', function() {
            ...
            if (jockey_optional_check) {
                ...
                $.each(adder_ids, function(key, value) {
                    ...
                    <td><input type="checkbox" name="adder_id" id="jockey-optional-${value.id}" class="adder-checkbox group1" data-id="${value.id}" value="${value.id}" ${checked_adder}></td>
                </tr>`;
                });
                $("#adder-optional-modal-table").html('');
        ...
        $(document).on('click', '#optional-button-add', function() {
            console.log(pump_type);
            if (pump_type === "electrical-diesel") {
                // Collect currently checked adder IDs from both tables
                const currentAdderIds = [];

                // Log all checkboxes to verify their states
                console.log('All Electrical Checkboxes:', $('.electrical-main-table input[name="adder_id"]').map(function() {
                    return { id: $(this).val(), checked: $(this).prop('checked') };
                }).get());
                console.log('All Diesel Checkboxes:', $('.diesel-main-table input[name="adder_id"]').map(function() {
                    return { id: $(this).val(), checked: $(this).prop('checked') };
                }).get());

                // Collect checked electrical adders
                $('.electrical-main-table input[name="adder_id"]').each(function() {
                    if ($(this).prop('checked')) {
                        currentAdderIds.push($(this).val());
                    }
                });

                // Collect checked diesel adders
                $('.diesel-main-table input[name="adder_id"]').each(function() {
                    if ($(this).prop('checked')) {
                        currentAdderIds.push($(this).val());
                    }
                });

                // Debug: Log checked adders
                console.log('Checked Electrical Adders:', $('.electrical-main-table input[name="adder_id"]').filter(function() { return $(this).prop('checked'); }).map(function() { return $(this).val(); }).get());
                console.log('Checked Diesel Adders:', $('.diesel-main-table input[name="adder_id"]').filter(function() { return $(this).prop('checked'); }).map(function() { return $(this).val(); }).get());
                console.log('Current Adder IDs (before update):', currentAdderIds);

                // Add new selections to adderIds, preserving existing ones
                const newAdderIds = currentAdderIds.filter(id => !adderIds.includes(id));
                adderIds = [...new Set([...adderIds, ...newAdderIds])];

                // Remove IDs that are no longer checked
                adderIds = adderIds.filter(id => currentAdderIds.includes(id));

                console.log('Updated adderIds:', adderIds);

                // Keep modal open
                // $("#adder-optional-modal").hide(); // Commented out
            }
                ...
                console.log('Updated adderIds (non-electrical-diesel):', adderIds);
        ...
        function changeElectricalDieselAdders() {
            $('.modal-heading-change').each(function() {
                const table = $('.' + $(this).data('type') + '-main-table');
                if ($(this).hasClass('active')) {
                    table.show();
                } else {
                    table.hide();
                }
                console.log('Checkbox states after switching to ' + $(this).data('type') + ':', $('.' + $(this).data('type') + '-main-table input[name="adder_id"]').map(function() {
                    return { id: $(this).val(), checked: $(this).prop('checked') };
                }).get());
            });
        }
        ...
        function dieselPumpPriceCalculate() {
            ...
            if (checkvalblank) {
                ...
                if (dieselSelectionData.length > 0) {
                    ...
                    alert('Diesel Pump Data not found test1');
                }
            }
        }
        ...
        function jockeyPumpPriceCalculate() {
            ...
            if (checkvalblank) {
                ...
                var jockeypump_article_no = $('input[name="jockey_article_number"]').val();
    ...
    <script>
        ...
        $('input[name="jockey_article_number"]').prop('disabled', true);
        $(document).on('blur', 'input[name="jockey_article_number"]', function() {
        ...
        $(document).on('blur', 'input[name="jockey_full_article_number"]', function() {///** start 20241231 for jockey pump form auto fill***********/
            ...
            if (jockeypump_article_no != '') {
                ...
                if ($.isArray(jockeypump_article_data) && jockeypump_article_data.length > 0) {
                    ...
                    var adderIdsfind = jockeypump_article_data[0].all_prices.adderpricelist;
                    ...
                    $("#error-modal-body").html('');
                    $("#error-modal-body").html('<h4>Jockey Pump data not found..!!</h4>');
                    $("#error-modal").show();
                    return;
                }
            }
        ...
        function jockeyPumpAjax() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    jockeypump = response;
                    $('input[name="jockey_article_number"]').prop('disabled', false);
                },
                error: function(data) {
                    console.log(data);
                }
