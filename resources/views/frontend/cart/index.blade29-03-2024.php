            ...
            <div class="cartMidSection">
                ...
                <div class="cartSection">
                    <div class="tableResponsive">
                        @if($controlPanelCartData->isNotEmpty() || $atmosCartData->isNotEmpty() ||  $scpCartData->isNotEmpty() || $boosterCartData->isNotEmpty() || $fireFightingCartData->isNotEmpty())
                        <table>
                            <thead>
                                <tr>
                                    <!--<th width="5%"></th>-->
...
<!--                                <button><img src="{{asset('fassets/images/downloadIcon.png')}}" /></button>-->
                            ...
                            <td class="total-price">
                                {{ App\Helpers\CurrencyHelper::withCurrency($val->price*$val->qty) }}</td>
                            <td>
                            ...
                            @if($fireFightingCartData->isNotEmpty())
                                @foreach($fireFightingCartData as $key=> $val)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="detail-modal-firefighting" href="javascript:void(0)">
                                                @php
                                                    $firefightingmodal = '';
...
//    Generate Quotation
    $("#generate-quotation").on("click", function () {
        ...
        if (cartIds.length >= 1 || atmosIds.length >= 1 || scpIds.length >= 1 || boosterIds.length >= 1 || firefightingIds.length >= 1) {


            var url = "{{url('controlpanel/customer-information?cp_ids=')}}" + cartIds + "&atmosIds=" + atmosIds + "&scpIds=" + scpIds + "&boosterIds=" + boosterIds + "&firefightingIds=" + firefightingIds;
            window.location = url;
        }
    });
